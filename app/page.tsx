    ...
    <main className="min-h-screen bg-[#030714]">
      ...
      <section className="relative pt-32 pb-20 px-4 overflow-hidden">
        ...
        <div className="relative max-w-3xl mx-auto text-center">
          ...
          <div className="max-w-lg mx-auto mb-16 animate-slide-up delay-200">
            <LocationSearch autoFocus placeholder="Land, Region oder Stadt suchen..." />
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 max-w-4xl mx-auto">
            {destinations.map((d) => <CountryCard key={d.name} {...d} />)}
          </div>
        </div>
      </section>
      ...
      <section className="py-12 px-4 border-t border-white/5">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex flex-wrap items-center justify-center gap-8 text-slate-600 text-sm font-medium">
            {['BBC', 'Reuters', 'WHO', 'GDACS', 'USGS', 'Auswärtiges Amt'].map(s => <span key={s} className="opacity-50 hover:opacity-80 transition">{s}</span>)}
          </div>
      ...
      <section className="py-20 px-4 border-t border-white/5">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid sm:grid-cols-3 gap-6 items-start">
            {tiers.map((tier) => (
              <div key={tier.name} className={`rounded-2xl p-6 md:p-8 transition-all duration-300 ${tier.popular ? 'backdrop-blur-xl bg-indigo-600/10 border-2 border-indigo-500/40 relative shadow-lg shadow-indigo-500/10 scale-[1.02]' : 'backdrop-blur-xl bg-white/5 border border-white/10 hover:border-white/20'}`}>
                ...
                <div className="mb-4"><span className="text-3xl font-bold">{tier.price}</span>{tier.period && <span className="text-slate-500">{tier.period}</span>}</div>
                <ul className="space-y-2 text-sm text-slate-300 mb-6">
                  {tier.features.map(f => <li key={f} className="flex items-center gap-2"><span className="text-indigo-400 flex-shrink-0">✓</span>{f}</li>)}
                </ul>
                <Link href={tier.popular ? '/pricing' : '/dashboard'} className={`block text-center py-3 rounded-xl font-semibold text-sm transition-all ${tier.popular ? 'bg-gradient-to-r from-indigo-600 to-violet-600 text-white hover:from-indigo-500 hover:to-violet-500 shadow-lg shadow-indigo-500/25' : 'bg-white/5 text-slate-300 hover:bg-white/10 border border-white/10'}`}>
      ...
      <section className="py-20 px-4 border-t border-white/5">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <Link href="/dashboard" className="inline-flex items-center gap-2 bg-gradient-to-r from-indigo-600 to-violet-600 text-white font-semibold px-8 py-4 rounded-xl hover:from-indigo-500 hover:to-violet-500 transition-all shadow-lg shadow-indigo-500/25 text-lg">🚀 Kostenlos testen</Link>
        </div>
      </section>

      <footer className="border-t border-white/5 py-8 px-4">
        <div className="max-w-4xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2 font-bold text-slate-300">🛡️ Risk<span className="text-indigo-400">Vector</span></div>
